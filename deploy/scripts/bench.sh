...
# Cross-agent memory: a subagent writes a fact; a FRESH orchestrator session recalls it -- proving
...
case "${1:-all}" in
  ...
  cap)        STEP_TOTAL=1; preflight; cap_run; finish ;;
  xagent)     STEP_TOTAL=1; preflight; xagent_run; finish ;;
  ...
  all)        STEP_TOTAL=18; preflight
              ...
              cap_run           || warn "cap step errored"
              xagent_run        || warn "xagent step errored"
              longctx_fool      || warn "longctx step errored"
              memory_ab         || warn "memory A/B step errored"
              finish ;;
  *) die "usage: bench.sh {all|quick|capability|code|safety|longctx|speed|e2e|prune|cap|xagent|memory}   (SIZE=smoke|small|large|max)" ;;
esac
