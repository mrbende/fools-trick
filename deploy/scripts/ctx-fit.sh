...
# config.yaml (worker.ctx_per_slot) and the opencode magus limit.context to match, then restart.
...
if [ -n "$BEST" ]; then
  ...
  say "to adopt: set worker.ctx_per_slot=$BEST in config.yaml AND limit.context=$BEST for magus in opencode.json (parity is test-guarded), then: make worker-up"
else
